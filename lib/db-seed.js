...
export async function seedDatabase() {
  try {
    // 1. Check if database has profiles
    const profileCountRows = await query("SELECT COUNT(*) as count FROM profiles");
    const profileCount = profileCountRows[0]?.count || 0;

    if (profileCount > 0) {
      console.log("Database already seeded. Skipping.");
      return { seeded: false, message: "Database already contains profiles." };
    }
    ...
    const ccisDeptId = "d1d1d1d1-1111-4111-d111-111111111111";
    const coeDeptId = "d2d2d2d2-2222-4222-d222-222222222222";

    const ccis101Id = "c1c1c1c1-1111-4111-c111-111111111111";
    const ccis102Id = "c2c2c2c2-2222-4222-c222-222222222222";
    const ccis103Id = "c3c3c3c3-3333-4333-c333-333333333333";
    const ccis104Id = "c4c4c4c4-4444-4444-c444-444444444444";
    const coe101Id = "e1e1e1e1-1111-4111-e111-111111111111";
    const coe102Id = "e2e2e2e2-2222-4222-e222-222222222222";
    ...
    await query(
      ...
      [ccisDeptId, "College of Computer and Information Sciences (CCIS)"]
    );
    await query(
      "INSERT INTO departments (id, name) VALUES (?, ?)",
      [coeDeptId, "College of Engineering (COE)"]
    );

    // 4. Insert Courses
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [ccis101Id, "CCIS101", "Introduction to Computing", 3, ccisDeptId]
    );
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [ccis102Id, "CCIS102", "Computer Programming 1", 3, ccisDeptId]
    );
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [ccis103Id, "CCIS103", "Data Structures and Algorithms", 3, ccisDeptId]
    );
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [ccis104Id, "CCIS104", "Database Management Systems", 3, ccisDeptId]
    );
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [coe101Id, "COE101", "Engineering Mathematics 1", 4, coeDeptId]
    );
    await query(
      "INSERT INTO courses (id, code, title, units, department_id) VALUES (?, ?, ?, ?, ?)",
      [coe102Id, "COE102", "Technical Drawing", 2, coeDeptId]
    );

    // 5. Enroll Student in Courses
    await query(
      "INSERT INTO enrollments (id, user_id, course_id) VALUES (uuid(), ?, ?), (uuid(), ?, ?), (uuid(), ?, ?), (uuid(), ?, ?)",
      [studentId, ccis101Id, studentId, ccis102Id, studentId, ccis103Id, studentId, coe101Id]
    );
    ...
    await query(
      ...
      [syllabusCCIS101, ccis101Id, instructorId]
    );
    ...
    await query(
      ...
      [syllabusCCIS102, ccis102Id, instructorId]
    );
    ...
    await query(
      ...
      [syllabusCCIS103, ccis103Id, instructorId]
    );
